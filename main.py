# onefile3.py - PART 1/4
# Wordle-like app (4/5/6 + Daily) patched: signup, stats, and per-row disabling on Enter.
import sys
import os
import sqlite3
import random
...
# onefile3.py - PART 2/4
# Login, Signup (patched), Statistic (patched), BaseWordle helpers and coloring logic
...
class SignupUI(object):
    def setupUi(self, Dialog):
        ...
        self.title.setGeometry(QtCore.QRect(160, 20, 120, 40))
        f = QFont(); f.setPointSize(20); f.setBold(True); self.title.setFont(f)
        ...
        self.lbl_user.setGeometry(QtCore.QRect(20, 20, 100, 16)); self.lbl_user.setText("Username:")
        ...
        self.lbl_pw.setGeometry(QtCore.QRect(20, 82, 100, 16)); self.lbl_pw.setText("Password:")
        self.pw_edit = QtWidgets.QLineEdit(self.groupBox)
        self.pw_edit.setGeometry(QtCore.QRect(20, 102, 260, 34)); self.pw_edit.setEchoMode(QtWidgets.QLineEdit.Password)
        # Confirm Password
        self.lbl_confirm = QtWidgets.QLabel(self.groupBox)
        self.lbl_confirm.setGeometry(QtCore.QRect(20, 144, 140, 16)); self.lbl_confirm.setText("Confirm Password:")
        self.pw_confirm = QtWidgets.QLineEdit(self.groupBox)
        self.pw_confirm.setGeometry(QtCore.QRect(20, 164, 260, 34)); self.pw_confirm.setEchoMode(QtWidgets.QLineEdit.Password)
        # Buttons
        self.signup_btn = QtWidgets.QPushButton(self.groupBox)
        self.signup_btn.setGeometry(QtCore.QRect(60, 210, 80, 34)); self.signup_btn.setText("Sign-up"); self.signup_btn.clicked.connect(self.save_user)
        self.cancel_btn = QtWidgets.QPushButton(self.groupBox)
        self.cancel_btn.setGeometry(QtCore.QRect(160, 210, 80, 34)); self.cancel_btn.setText("Cancel"); self.cancel_btn.clicked.connect(self.back_to_login)
...
class FourWordle(BaseWordle):
    ...
    def enter(self):
        ...
        # disable only this row now that it has been submitted/locked
        for w in widgets:
            if w:
                w.setEnabled(False)
...
class DailyWordUI(BaseWordle):
    ...
    def setupUi(self, MainWindow):
        ...
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.title = QtWidgets.QLabel(self.centralwidget)
        self.title.setGeometry(QtCore.QRect(360,10,260,40))
        f = QFont(); f.setPointSize(16); f.setBold(True)
        self.title.setFont(f)
        self.title.setText("Daily Word")

        self.widget = QtWidgets.QWidget(self.centralwidget)
        self.widget.setGeometry(QtCore.QRect(40,80,820,360))
        self.grid = QtWidgets.QGridLayout(self.widget)
